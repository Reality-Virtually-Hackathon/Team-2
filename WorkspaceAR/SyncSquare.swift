...
class SyncSquare: SCNNode {
	...
	private var recentSyncSquarePositions: [float3] = []
	
	/// Previously visited plane anchors.
	private var anchorsOfVisitedPlanes: Set<ARAnchor> = []
	
	/// List of the segments in the focus square.
	private var segments: [SyncSquare.Segment] = []
	
	/// The primary node that controls the position of other `SyncSquare` nodes.
	...
	override init() {
		...
		*/
		...
		let c: Float = SyncSquare.thickness / 2 // correction to align lines perfectly
		...
		positioningNode.simdScale = float3(SyncSquare.size * SyncSquare.scaleForClosedSquare)
	...
	private func displayAsOpen(at position: float3, camera: ARCamera?) {
		performOpenAnimation()
		recentSyncSquarePositions.append(position)
	...
	private func updateTransform(for position: float3, camera: ARCamera?) {
		...
		recentSyncSquarePositions = Array(recentSyncSquarePositions.suffix(10))
		
		// Move to average of recent positions to avoid jitter.
		let average = recentSyncSquarePositions.reduce(float3(0), { $0 + $1 }) / Float(recentSyncSquarePositions.count)
	...
	*/
	...
	private func performOpenAnimation() {
		...
		SCNTransaction.animationDuration = SyncSquare.animationDuration / 4
		...
		SCNTransaction.animationTimingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseOut)
		SCNTransaction.animationDuration = SyncSquare.animationDuration / 4
		positioningNode.simdScale = float3(SyncSquare.size)
		SCNTransaction.commit()
	}
	...
	private func performCloseAnimation(flash: Bool = false) {
		...
		SCNTransaction.begin()
		SCNTransaction.animationTimingFunction = CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseOut)
		SCNTransaction.animationDuration = SyncSquare.animationDuration / 2
		...
		if flash {
			let waitAction = SCNAction.wait(duration: SyncSquare.animationDuration * 0.75)
			let fadeInAction = SCNAction.fadeOpacity(to: 0.25, duration: SyncSquare.animationDuration * 0.125)
			let fadeOutAction = SCNAction.fadeOpacity(to: 0.0, duration: SyncSquare.animationDuration * 0.125)
			fillPlane.runAction(SCNAction.sequence([waitAction, fadeInAction, fadeOutAction]))
			
			let flashSquareAction = flashAnimation(duration: SyncSquare.animationDuration * 0.25)
			for segment in segments {
	...
	private func scaleAnimation(for keyPath: String) -> CAKeyframeAnimation {
		...
		let size = SyncSquare.size
		let ts = SyncSquare.size * SyncSquare.scaleForClosedSquare
		...
		scaleAnimation.duration = SyncSquare.animationDuration
	...
	private lazy var fillPlane: SCNNode = {
		let correctionFactor = SyncSquare.thickness / 2 // correction to align lines perfectly
		let length = CGFloat(1.0 - SyncSquare.thickness * 2 + correctionFactor)
		...
		material.diffuse.contents = SyncSquare.fillColor
		material.isDoubleSided = true
		material.ambient.contents = UIColor.black
		material.lightingModel = .constant
		material.emission.contents = SyncSquare.fillColor
...
extension SyncSquare.State: Equatable {
	static func ==(lhs: SyncSquare.State, rhs: SyncSquare.State) -> Bool {
		switch (lhs, rhs) {
